    ...
    if (!fs.pathExistsSync('./.git')) {
        StdOutUtil.printError(
            'You are not in a git root directory: this command will only deploy the current directory.\n' +
                'Run "alacrity deploy --help" to know more deployment options... (e.g. tar file or image name)\n',
            true
        )
    }
    if (!commandExistsSync('git')) {
        StdOutUtil.printError(
            '"git" command not found: AlaCrity needs "git" to create tar file from your branch source files.\n' +
                'Run "alacrity deploy --help" to know more deployment options... (e.g. tar file or image name)\n',
            true
        )
    }
...
export function validateDefinitionFile() {
    if (!fs.pathExistsSync('./alacran-definition')) {
        if (fs.pathExistsSync('./Dockerfile')) {
            StdOutUtil.printWarning('**** Warning ****')
            StdOutUtil.printMessage(
                'No alacran-definition was found in main directory: falling back to Dockerfile.\n'
            )
        } else {
            StdOutUtil.printWarning('**** Warning ****')
            StdOutUtil.printMessage(
                'No alacran-definition was found in main directory: unless you have specified a special path for your alacran-definition, this build will fail!\n'
            )
        }
    } else {
        let content = null
        try {
            content = JSON.parse(
                fs.readFileSync('./alacran-definition', 'utf8')
            )
        } catch (e) {
            StdOutUtil.printError(
                `alacran-definition file is not a valid JSON!\n${
                    e.message || e
                }\n`,
                true
            )
        ...
        if (!content || !content.schemaVersion) {
            StdOutUtil.printError(
                'alacran-definition needs "schemaVersion": please see docs!\n',
                true
            )
        }
    }
...
export function getErrorForMachineName(
    ...
    if (checkExisting && !exist) {
        return `${StdOutUtil.getColoredMachineName(
            value
        )} AlaCrity machine does not exist.`
    }
    if (checkExisting || isNameValid(value)) {
        return true
    }
    return 'Please enter a valid AlaCrity machine name: small letters, numbers, single hyphen.'
}
...
export function getErrorForAppName(
    ...
    if (!app) {
        return `${StdOutUtil.getColoredAppName(
            value
        )} app does not exist on this AlaCrity machine.`
    }
