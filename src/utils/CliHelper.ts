...
export default class CliHelper {
    ...
    findDefaultAlacranName() {
        let currentSuffix = 1
        const machines = StorageHelper.get()
            .getMachines()
            .map((machine) => machine.name)
        while (machines.includes(this.getAlacranFullName(currentSuffix))) {
            currentSuffix++
        }
        return this.getAlacranFullName(currentSuffix)
    }

    getAlacranFullName(suffix: number) {
        return `alacran-${suffix < 10 ? '0' : ''}${suffix}`
    }
