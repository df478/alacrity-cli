...
export default class Deploy extends Command {
    ...
    protected usage =
        ...
        '  Use --alacrityName to use an already logged in AlaCrity machine\n' +
        '  Use --alacrityUrl and --alacrityPassword to login on the fly to a AlaCrity machine, if also --alacrityName is present, login credetials are stored locally\n' +
        '  Use one among --branch, --tarFile, --imageName'

    protected description =
        "Deploy your app to a specific AlaCrity machine. You'll be prompted for missing parameters."
    ...
    protected options = (params?: IParams): IOption[] => [
        ...
        },
        {
            ...
            env: 'ALACRITY_NAME',
            message: params
                ? 'select the AlaCrity machine name you want to deploy to'
                : 'AlaCrity machine name, to load/store credentials',
            type: 'list',
        ...
        },
        {
            ...
            type: 'input',
            default:
                params && (process.env.ALACRITY_DEFAULT_BRANCH || 'master'),
            when:
        ...
        },
        {
            name: K.tar,
            char: 't',
            env: 'ALACRITY_TAR_FILE',
            message:
                'tar file to be uploaded, must contain alacran-definition file',
            type: 'input',
            when: false
        },
        {
            name: K.img,
            char: 'i',
            env: 'ALACRITY_IMAGE_NAME',
            message:
                'image name to be deployed, it should either exist on server, or it has to be public, or on a private repository that AlaCrity has access to',
            type: 'input',
            when: false
        },
        {
    ...
    protected async preAction(
        ...
        StdOutUtil.printMessage('Preparing deployment to AlaCrity...\n')
                ...
                if (
                    ...
                    StdOutUtil.printError(
                        `You have to first login to ${StdOutUtil.getColoredMachineName(
                            possibleApp.machineNameToDeploy
                        )} AlaCrity machine to use previously saved deploy options from this directory with --default.\n`,
                        true
                    )
                }
        ...
        try {
            if (
                ...
                StorageHelper.get().saveDeployedDirectory({
                    ...
                    machineNameToDeploy: deployParams.alacranMachine
                        ? deployParams.alacranMachine.name
                        : ''
                })
            }
        } catch (error) {
            ...
            StdOutUtil.printError(
                `\nSomething bad happened: cannot deploy ${StdOutUtil.getColoredAppName(
                    ...
                    deployParams.alacranMachine
                        ? deployParams.alacranMachine.name ||
                              deployParams.alacranMachine.baseUrl
                        : ''
                )}.\n${errorMessage}\n`,
                true
            )
