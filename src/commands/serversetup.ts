...
export default class ServerSetup extends Command {
    ...
    protected description =
        'Performs necessary actions to prepare AlaCrity on your server.'
    ...
    protected options = (params?: IParams): IOption[] => [
        this.getDefaultConfigFileOption(() => this.preQuestions(params!)),
        {
            ...
            message: () =>
                ...
                ' already started AlaCrity container on your server' +
            ...
            preProcessParam: (param?: IParam) => {
                if (param && !param.value) {
                    StdOutUtil.printError(
                        '\nCannot setup AlaCrity if container is not started!\n'
                    )
                    StdOutUtil.printWarning(
                        'Start it by running the following line:'
                    )
                    StdOutUtil.printMessage(
                        'docker run -p 80:80 -p 443:443 -p 3000:3000 -v /var/run/docker.sock:/var/run/docker.sock -v /alacran:/alacran alacrity/alacrity'
                    )
                    StdOutUtil.printMessage(
                        '\nPlease read tutorial on AlaCrity.com to learn how to install AlaCrity on a server.\n',
                        true
                    )
                }
            }
        ...
        },
        {
            ...
            type: 'password',
            message: 'current AlaCrity password',
        ...
        },
        {
            name: K.root,
            char: 'r',
            env: 'ALACRITY_ROOT_DOMAIN',
            aliases: [{ name: 'rootDomain', hide: true }],
            type: 'input',
            message: 'AlaCrity server root domain',
        ...
        },
        {
            name: K.newPwd,
            char: 'w',
            env: 'ALACRITY_NEW_PASSWORD',
            type: 'password',
            message: `new AlaCrity password (min ${Constants.MIN_CHARS_FOR_PASSWORD} characters)`,
        ...
        },
        {
            name: K.newPwdCheck,
            type: 'password',
            message: 'enter new AlaCrity password again',
            hide: true,
        ...
        },
        {
            name: K.email,
            char: 'e',
            env: 'ALACRITY_CERTIFICATE_EMAIL',
    ...
    protected async preAction(
        ...
        StdOutUtil.printMessage('Setup AlaCrity machine on your server...\n')
    ...
    private async getAuthTokenFromIp(firstTry?: boolean): Promise<string> {
        try {
            ...
            if (
                firstTry &&
                e.alacranStatus === ErrorFactory.STATUS_WRONG_PASSWORD
            ) {
                return ''
            }
            if ((e + '').indexOf('Found. Redirecting to https://') >= 0) {
                StdOutUtil.printWarning(
                    '\nYou may have already setup the server! Use alacrity login to log into an existing server.'
                )
            } else {
                StdOutUtil.printWarning(
                    '\nYou may have specified a wrong IP address or not already started AlaCrity container on your server!'
                )
            }
    ...
    private async checkFreshInstallation(): Promise<boolean> {
        try {
            ...
            if (rootDomain) {
                StdOutUtil.printWarning(
                    `\nYou may have already setup the server with root domain: ${rootDomain}! Use alacrity login to log into an existing server.`,
                    true
                )
            }
        } catch (e) {
    ...
    private async updateRootDomain(rootDomain: string) {
        try {
            ...
            if (e.alacranStatus === ErrorFactory.VERIFICATION_FAILED) {
                StdOutUtil.printError(
    ...
    private async enableSslAndChangePassword(
        ...
        try {
            ...
            if (forcedSsl) {
                ...
                StdOutUtil.printError(
                    `Then use <alacrity login> command to connect to your server.`
                )
            }
            SpinnerHelper.fail()
            StdOutUtil.errorHandler(e)
    ...
    protected async action(params: IParams): Promise<void> {
        ...
        StdOutUtil.printGreenMessage(
            `AlaCrity server setup completed: it is available as ${StdOutUtil.getColoredMachine(
                this.machine
            )}\n`
        )
        StdOutUtil.printMessage('For more details and docs see AlaCrity.com\n')
    }
}
